...
# 5. Calculate the percentage of two types. Either one of both is larger than set threshold (default 90%), that combination is the haplotype.
# 6. If both of them are smaller than threshold, skip the lower line. Check for the first line with the next following line.
# 7. If skipped "lower" lines beyond set times (default 10), skip the first line, and start with the first skipped line again.
...
parser.add_option("-n", "--nextline", dest="next", type = "int", default = 10, help="Maximum number of next line skipped")
